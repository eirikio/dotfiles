...
$mwslPrincipal = New-ScheduledTaskPrincipal -UserId "$env:USERNAME" -LogonType Interactive -RunLevel Highest
...
$adminPrincipal = New-ScheduledTaskPrincipal -UserId "$env:USERNAME" -LogonType Interactive -RunLevel Highest
Register-ScheduledTask -Action $adminAction -Trigger $adminTrigger -Principal $adminPrincipal -TaskName "RunAfterReboot" -Force

Write-Host "`n✅ Tasks registered. They will run on next login."
